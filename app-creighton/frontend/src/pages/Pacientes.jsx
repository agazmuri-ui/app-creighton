  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <h2 className="modal-title">{paciente ? 'Editar paciente' : 'Nuevo caso'}</h2>
        ...
        <div className="form-group">
          ...
          <textarea value={form.notas} onChange={e => setForm({ ...form, notas: e.target.value })} placeholder="Observaciones del caso..." />
        </div>
...
export default function Pacientes() {
  ...
  async function eliminar(id, e) {
    e.stopPropagation();
    if (!confirm('¿Eliminar este caso? Se borrarán todas las sesiones.')) return;
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-sub">{activos} casos activos · {pacientes.length} total</p>
        </div>
      ...
      <div className="card">
        {loading ? (
          ...
          <div className="empty">
            ...
            <div className="empty-text">{filtro ? 'Sin resultados' : 'Sin casos. Agrega el primero.'}</div>
          </div>
        ) : (
